...
def get_current_datetime() -> str:
    '''
    Получение текущей даты и времени для сохранения
    '''
    return datetime.datetime.now().strftime("%I-%M%p_%B_%d_%Y_")
...
def run(
    ...
    # Directories
    save_dir = increment_path(Path(project) / f"{name}_{get_current_datetime()}", exist_ok=exist_ok)  # increment run
